...
def calc():
    ...
    u = 1* phif
    p1 = np.arange(s,r,u)
    plt.title('VARIATION OF THRESHOLD VOLTAGE WITH PHIF')
    plt.plot(p1,thresvol(p1,v1),label='First Vsb:'+str(v1))
    plt.plot(p1,thresvol(p1,v2),label='Second Vsb:'+str(v2))
    plt.plot(p1,thresvol(p1,v3),label='Third Vsb:'+str(v3))
    plt.legend()
    plt.xlabel('Phif')
