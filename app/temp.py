...
def calc():
    ...
    def thresvol(T,Vsb):
        cox = eox / tox
        phif=(k*T/q)*np.log(Na/ni)
        gamma = (np.sqrt(2 * q * esi * Na)) / cox
        x=(Vsb)+2*phif
        a=np.sqrt(abs(x))
        b=np.sqrt(abs(2*phif))
        c=a-b
        Vth=vt+gamma*c
        return Vth
    ...
    T1 = np.arange(j,k,l)
    plt.title('VARIATION OF THRESHOLD VOLTAGE WITH Temperature')
    plt.plot(T1,thresvol(T1,v1),label='First Vsb:'+str(v1))
    plt.plot(T1,thresvol(T1,v2),label='Second Vsb:'+str(v2))
    plt.plot(T1,thresvol(T1,v3),label='Third Vsb:'+str(v3))
